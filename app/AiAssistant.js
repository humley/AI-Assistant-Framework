...
function HumleyAiAssistant(username, password, HumleyId) {
    ...
    this.humleyApi = new humleyConversationBot(this.credential);
    this.initAiAssistant = function (parentId) {
        ...
        vm.humleyApi.init(data).then(function (result) {
            if (!result) {
    ...
    this.addMessage = function (message) {
        var other = arguments.length <= 1 || arguments[1] === undefined ? false : arguments[1];
    ...
    this.submitMessage = function (e) {
        ...
        vm.humleyApi.talk(txt).then(function (data) {
            if (data.error) {
                console.error(data.errorInfo);
            }
            for (var i = 0; i < data.replies.length; i++) {
                if (data.replies[i].hasVideo) {
                    data.replies[i].reply += '</p><p style="margin-top:15px;"><iframe id="vpframe" src="' + data.replies[i].video.url + '" frameborder="0" allowfullscreen></iframe>';
                }
                if (data.replies[i].buttonId !== undefined) {
                    if (data.replies[i].buttonId === -1) {
                        var feedbackButtons = '<span id="buttonOption" class="buttonResponseShow"><button type="button" onclick="sendButtonResponse(\'Yes\', 3)" class="btn btn-success" style="margin-right:10px;">Yes</button><span id="buttonOption" class="buttonResponseShow"><button type="button" onclick="sendButtonResponse(\'Sort of\', 2)" class="btn btn-info" style="margin-right:10px;">Sort of</button><button type="button" onclick="sendButtonResponse(\'No\', 0)" class="btn btn-danger">No</button></span>';
                        data.replies[i].reply += '</p><p style="margin-top:15px;">' + feedbackButtons;
                    } else {
                        ...
                        if (actionButtonConent !== '') {
                            data.replies[i].reply += '</p><p style="margin-top:15px;">' + actionButtonConent;
                        }
                    }
                }
...
var api = new HumleyAiAssistant('Humley', '81mst3r', 141);
api.initAiAssistant('demo');
function checkpoint(bannerId, ActionType) {
    if (ActionType === 'Browser') {
        api.humleyApi.sendCheckPoint(6001, 'BannerId', bannerId);
    }
}
function sendButtonResponse(message, typeId) {
    api.humleyApi.sendFeedback(typeId);
    api.addMessage(document.createTextNode(message), true);
}
